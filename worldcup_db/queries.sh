...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON teams.team_id = games.game_id WHERE year=2018 AND round='Final';")"
...
echo "$($PSQL "SELECT DISTINCT(year), name FROM teams INNER JOIN games ON teams.team_id = games.game_id WHERE round='Final' ORDER by year;")"
